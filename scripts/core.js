...
// All interdependent functions kept together for stability
        ...
        async function duplicateTrip() {
            const confirmed = await customConfirm(
                'Duplicate Trip?',
                'Create a copy of this trip as a new template?\n\nThis will copy destinations, bookings, and checklists.',
                'Duplicate',
                'Cancel'
            );
            if (!confirmed) return;
        ...
        async function importData() {
            ...
            input.onchange = async (e) => {
                ...
                try {
                    ...
                    const confirmed = await customConfirm(
                        'Import Data?',
                        '⚠️ WARNING: This will OVERWRITE your current trip data!\n\nAll existing data will be replaced. Are you sure?',
                        'Import & Overwrite',
                        'Cancel'
                    );
                    if (!confirmed) return;
        ...
        async function onTripDateChange() {
            ...
            if (departure && returnDate) {
                ...
                if (retDate < depDate) {
                    await customAlert(
                        'Invalid Dates',
                        '⚠️ Return date cannot be before departure date!\n\nPlease choose a return date after ' + new Date(departure).toLocaleDateString(),
                        'warning'
                    );
                    // Reset to old value or clear
                    returnInput.value = tripData.overview.returnDate || '';
                    return;
                }
                
                // Warn if trip is very long (>90 days)
                const daysDiff = Math.ceil((retDate - depDate) / (1000 * 60 * 60 * 24));
                if (daysDiff > 90) {
                    const confirmed = await customConfirm(
                        'Long Trip',
                        `This trip is ${daysDiff} days long (over 3 months).\n\nIs this correct?`,
                        'Yes, Correct',
                        'Fix Dates'
                    );
                    if (!confirmed) {
        ...
        function updateBudgetEur(idx, eurValue) {
            ...
            if (eur < 0) {
                customAlert('Invalid Amount', '⚠️ Budget amount cannot be negative!\n\nPlease enter a positive number.', 'warning');
                // Reset to previous value
                const input = document.querySelector(`input[onchange*="updateBudgetEur(${idx}"]`);
                if (input) input.value = tripData.budget[idx].amountEur || 0;
                return;
            }
            
            // Warn if amount is suspiciously high (>10,000 EUR)
            if (eur > 10000) {
                customConfirm(
                    'Large Amount',
                    `This amount is €${eur.toLocaleString()}.\n\nIs this correct?`,
                    'Yes',
                    'Fix'
                ).then(confirmed => {
                    if (!confirmed) {
        ...
        async function exportTripAsPDF() {
            ...
            // Use the same clean export as exportToPDF() in Quick Actions
            const tripName = (tripData.overview.destination || 'My Trip').replace(/[^\w\s-]/g, '').trim();
            const departure = tripData.overview.departureDate;
            const returnDate = tripData.overview.returnDate;
            
            // Helper to clean text (remove emojis and special chars)
            const cleanText = (text) => {
                if (!text) return '';
                return String(text)
        ...
        async function deleteImportantInfo(id) {
            const confirmed = await customConfirm('Delete Information?', 'Remove this important information from the list?', 'Delete', 'Cancel');
            if (!confirmed) return;
        ...
        async function submitAddMember() {
            ...
            if (!name) {
                await customAlert('Name Required', 'Please enter a member name.', 'warning');
                return;
            }
        ...
        async function sendInvite() {
            ...
            if (!email) {
                await customAlert('Email Required', 'Please enter an email address.', 'warning');
                return;
            }
            ...
            if (!emailRegex.test(email)) {
                await customAlert('Invalid Email', '⚠️ Please enter a valid email address.\n\nExample: friend@example.com', 'warning');
                return;
            }
            
            // Prevent inviting yourself
            if (user && email.toLowerCase() === user.email?.toLowerCase()) {
                await customAlert('Cannot Invite Yourself', "You're already part of this trip! 😊", 'info');
                return;
            }
        ...
        async function deleteChecklistItem(idx) {
            const confirmed = await customConfirm(
                ...
                '⚠️ This will remove it for EVERYONE in the group.\n\nAre you sure?',
                'Delete for Everyone',
                'Cancel'
            );
            if (!confirmed) return;
        ...
        async function optimizeMainRoute() {
            ...
            const confirmed = await customConfirm(
                'Optimize Route?',
                `Automatically reorder ${withCoords.length} destinations for the shortest route?\n\nThis will rearrange your main destinations by distance.`,
                'Optimize',
                'Cancel'
            );
            if (!confirmed) return;
        ...
        function addDayPlan() {
            ...
            tripData.dayPlans.push({ 
                ...
                collaborativeNotes: '' // Shared notes for this day
            });
            renderDayPlans();
            saveData();
        }
        ...
        function renderAll() {
            ...
            requestAnimationFrame(() => {
                ...
                if (tripData.overview) {
                    ...
                    const retEl = document.getElementById('returnDate');
                    if (destEl && destEl.value !== tripData.overview.destination) {
                        destEl.value = tripData.overview.destination || '';
                    }
                    if (depEl && depEl.value !== tripData.overview.departureDate) {
                        depEl.value = tripData.overview.departureDate || '';
                    }
                    if (retEl && retEl.value !== tripData.overview.returnDate) {
                        retEl.value = tripData.overview.returnDate || '';
                    }
                }
                ...
                // Fetch weather for current destination (debounced)
                if (tripData.overview.destination) {
                    debounce(() => fetchWeather(), 2000)();
                }
        ...
        window.addEventListener('load', async () => {
            ...
            try{
                ...
                if (shareToken) {
                    ...
                    // Disable all inputs
                    setTimeout(() => {
                        // Add CSS for guest mode and light theme
                        const style = document.createElement('style');
                        style.textContent = `
        ...
        async function submitUsernameChange() {
            ...
            if (!/^[a-zA-Z0-9_]{3,20}$/.test(newUsername)) {
                await customAlert('Invalid Username', 'Use 3-20 characters: letters, numbers, and underscore only.', 'warning');
                return;
            }
            ...
            try {
                ...
                if (existing) {
                    await customAlert('Username Taken', 'This username is already taken. Please choose another.', 'warning');
                    return;
                }
